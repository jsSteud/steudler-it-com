...
socket.on('state', (data) => {
  ...
  else if (currentRole)                 { renderReferee(); updateRefereeProgress(); }
});

socket.on('rated', ({ competitorId, refereeId, score }) => {
  state.ratings[competitorId][refereeId] = score;
  if (currentRole === 'committee') {
    if (sortByTotal) renderCommittee();
    else             updateCommitteeRow(competitorId);
    updateProgressChips();
  } else if (currentRole === refereeId) {
    updateRefereeRow(competitorId, false);
...
function saveScore(id) {
  ...
  socket.emit('rate', { competitorId: id, refereeId: currentRole, score });
}

function updateRefereeRow(id, skipActiveInput = true) {
  const score    = state.ratings[id]?.[currentRole];
  const hasScore = score !== null && score !== undefined;
  const input    = document.getElementById(`si-${id}`);
  const dot      = document.getElementById(`sd-${id}`);

  if (input && (!skipActiveInput || document.activeElement !== input)) {
    input.value     = hasScore ? score : '';
    input.className = 'score-input' + (hasScore ? ' saved' : '');
  }
...
function updateCommitteeRow(id) {
  const existing = document.getElementById(`kom-row-${id}`);
  if (!existing) return;
  const newRow = buildCommitteeRow(id, null);
